...
use Core\Helpers\Zipper\RepositoryInterface;
...
class Zipper
{
    ...
    public function make($pathToFile, $type = 'zip')
    {
        ...
        $objectOrName = $type;
        if (is_string($type)) {
            $objectOrName = 'Core\Helpers\Zipper\\' . ucwords($type) . 'Repository';
        }

        if (!is_subclass_of($objectOrName, 'Core\Helpers\Zipper\RepositoryInterface')) {
