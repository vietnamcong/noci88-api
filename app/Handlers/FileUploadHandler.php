...
class FileUploadHandler
{
	...
	protected $img_allowed_ext = ['svg', 'png', 'jpg', 'gif', 'jpeg', 'ico'];
	protected $img_max_size = 3 * 1024 * 1024;

	protected $file_max_size = 3 * 1024 * 1024;
	...
	public function uploadFile($file, $category = "tmp")
	{
		...
		try {
			$relative_file_path = $this->getStorage()->putFileAs($relative_folder_path, $file, $filename);
			// 将attachment的数据保存
			$insert_data = $this->getAttachmentInsertData($file, "file", $category, $relative_file_path);
			// 判断是否需要剪裁图片
			$model_data = Attachment::create($insert_data);
	...
	public function getAttachmentInsertData($file, $file_type, $category, $relative_file_path)
	{
		...
		return [
			"model_type" => auth()->check() ? \get_class($this->model) : 'App\Models\User',
			"model_id" => auth()->check() ? $this->model->id : 1,
			"ip" => get_client_ip(),
			"file_type" => $file_type,
